...
**每 60 秒**自動檢查 Accupass 的**指定日期場次**，**一旦釋出名額（從『已售完』變回可報名），
...
目前鎖定：**2026/06/13（六）整天 13 個場次**，任一場釋出名額就通知。

> ⏱️ **為什麼是每 60 秒、不是用 GitHub 排程？** GitHub 的 cron 排程實際會被降速到
> 每 1～3 小時才跑一次，搶票根本來不及。所以改成「單次執行內部自己每 60 秒檢查一圈、
> 連續跑約 5.5 小時」，結束後由下一次觸發接力。為了讓它能近乎 24 小時免費長跑，
> **這個 repo 設為 public**（Actions 分鐘數無限免費；程式不含祕密，webhook 仍在加密 Secret）。
