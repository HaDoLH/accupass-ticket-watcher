...
每 5 分鐘自動檢查一個 Accupass 活動頁，**一旦出現可報名的票，就把通知推到你的手機（Discord）**。
跑在 GitHub 的雲端伺服器上，**不用開電腦**，週末出門也能監看。

監看目標：SUPER JUNIOR 20TH ANNIVERSARY POP-UP
`https://www.accupass.com/event/2605080529051188996723`
...
GitHub Actions（每 5 分鐘）
        │
        ▼
watch_ticket.py ──► Playwright 開無頭瀏覽器、渲染頁面
        │
        ▼
讀畫面可見文字，找「立即報名」按鈕
        │
...
## 想改成監看別場活動？
打開 `watch_ticket.py`，把最上面 `TARGET_URL` 換成新的活動網址即可。